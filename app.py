...
DISEASE_JSON_PATH = 'ladiesfinger_disease_info.json'
...
model.to(device)
model.eval()
print("🧠 Ladiesfinger EfficientNet model loaded.")
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Ladiesfinger Disease Detection",
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🌴 New Ladiesfinger Disease Detection Result",
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['tdaitech@gmail.com'])
        
        # Create email body with prediction results
        email_body = f"""
        🔍 Ladiesfinger Disease Detection Result
