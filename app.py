...
MODEL_PATH = 'model/ladiesfinger_effnet.pth'  # Changed to ladies finger model
DISEASE_JSON_PATH = 'ladiesfinger_disease_info.json'  # Changed to ladies finger disease info
...
model.to(device)
model.eval()
print("🧠 Ladies Finger EfficientNet model loaded.")  # Updated print message
...
def send_daily_report():
    if daily_stats["count"] == 0:
        return
    try:
        msg = Message("📊 Daily Click Report - Ladies Finger Disease Detection",  # Updated email subject
...
def send_prediction_result_email(filename, prediction_results, image_path=None):
    try:
        msg = Message("🥬 New Ladies Finger Disease Detection Result",  # Updated email subject
                      sender=app.config['MAIL_USERNAME'],
                      recipients=['cropai2025@gmail.com'])
...
def extract_disease_details(disease_data, label):
    ...
    return {
        ...
        "urdu": {
            ...
            "پانی": urdu_water,
            "دوا": urdu_medicine,
            "نامी दवा": urdu_organic_medicine,
...
def send_email():
    ...
    if not (name and email and msgt):
        ...
        return redirect('/')
    try:
        m = Message("🥬 New Contact Request - Ladies Finger Disease Detection",  # Updated email subject
                    sender=app.config['MAIL_USERNAME'], 
        ...
        r.body = f"Hi {name},\nWe received your message regarding ladies finger plant disease detection."
        mail.send(r)
